...
public class SignatureHelper {
    ...
    public String createCanonicalRequest(final URI uri,
                                         ...
                                         final Map<String, List<String>> preSignedHeaders) throws AmazonClientException {
     ...
     */
    public Map<String, List<String>> createPreSignedHeaders(final URI uri, final Map<String, String> header) throws AmazonClientException {
        ...
        dateHeaderValue.add(V2Util.getFormattedTimestamp());
    ...
    public String getSignedHeadersString(final Map<String, List<String>> preSignedHeaders) {
        ...
        for (String header : sortedHeaders) {
            if (buffer.length() > 0)
                buffer.append(";");
            buffer.append(V2Util.lowerCase(header));
        }
    ...
    public String getCanonicalizedHeaderString(final Map<String, List<String>> preSignedHeaders) {
        ...
        final StringBuilder buffer = new StringBuilder();
        for (String header : sortedHeaders) {
            String key = V2Util.lowerCase(header);
     ...
     * @throws AmazonClientException When an error response is returned by Amazon Pay due to bad request or other issue
     */
    public String getCanonicalizedQueryString(final Map<String, List<String>> parameters) throws AmazonClientException {
        ...
        if (parameters != null) {
            for (Map.Entry<String, List<String>> entry : parameters.entrySet()) {
                final String encodedParamName = V2Util.urlEncode(
                        entry.getKey(), false);
                final List<String> paramValues = entry.getValue();
                final List<String> encodedValues = new ArrayList<String>(
                        paramValues.size());
                for (String value : paramValues) {
                    encodedValues.add(V2Util.urlEncode(value, false));
                }
     ...
     * @throws AmazonClientException
     */
    private String getCanonicalizedURI(final String path) throws AmazonClientException {
        if (path == null || path.isEmpty()) {
            return "/";
        } else {
            String value = V2Util.urlEncode(path, true);
            if (value.startsWith("/")) {
                return value;
            } else {
                return "/".concat(value);
